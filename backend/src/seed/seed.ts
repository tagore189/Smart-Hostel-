...
export const runSeed = async (shouldCloseDB: boolean = false) => {
  ...
  await OutingRequest.create({
    ...
    destination: 'Inorbit Mall & Durgam Cheruvu Lake Walk',
    reason: 'Personal shopping and dinner with college friends',
    status: 'APPROVED',
    approvedByName: 'Auto-Approved (Hostel Policy)',
    approvedAt: new Date(),
    gatePassCode: samplePassCode,
    qrPayload: JSON.stringify({
      passCode: samplePassCode,
      residentName: resident.name,
      roomNumber: '204',
      destination: 'Inorbit Mall',
      validFrom: '2026-09-24 18:30',
      validTo: '2026-09-24 21:00',
  ...
  await GatePass.create({
    outingRequest: new mongoose.Types.ObjectId(),
    resident: resident._id,
    passCode: samplePassCode,
    passType: 'OUTING',
    validFrom: new Date('2026-09-24T18:30:00'),
    validTo: new Date('2026-09-24T21:00:00'),
    status: 'ACTIVE',
  });

  // 12. Sample Visitor Entry
  const sampleVisitorPassCode = 'VP-451204-204';
  await Visitor.create({
    ...
    purpose: 'Weekend drop-in with homemade snacks and books',
    visitDate: '2026-09-26',
    arrivalTime: '11:00 AM',
    expectedDepartureTime: '04:00 PM',
    status: 'APPROVED',
    approvedByName: 'Instant Verification (Immediate Family)',
    visitorPassCode: sampleVisitorPassCode,
    qrPayload: JSON.stringify({
      passCode: sampleVisitorPassCode,
      visitorName: 'Sunita Sharma',
      relationship: 'Mother',
